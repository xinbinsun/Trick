...
def _request_wrapper(
    method: HTTP_METHOD_T, url: str, *, follow_relative_redirects: bool = False, proxies: Optional[Dict] = proxies, **params
) -> requests.Response:
    ...
    if follow_relative_redirects:
        ...
        if 300 <= response.status_code <= 399:
            ...
            if parsed_target.netloc == "":
                ...
                return _request_wrapper(method=method, url=next_url, follow_relative_redirects=True, proxies=proxies, **params)
        return response

    # Perform request and return if status_code is not in the retry list.
    response = requests.request(method=method, url=url, proxies=proxies, **params)
